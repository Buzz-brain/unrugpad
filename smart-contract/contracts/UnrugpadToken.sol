...
contract UnrugpadToken is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    uint256 public constant MAX_FEE_PERCENTAGE = 1500;
    uint256 public constant DENOMINATOR = 10000;
    uint256 public constant PLATFORM_FEE = 30; // 0.3% on every buy/sell
    uint256 public constant MAX_TRANSACTION_AMOUNT = 50;
    uint256 public constant MAX_WALLET_AMOUNT = 50;
    ...
    struct TokenConfig {
        ...
        uint256 buyFee;
        uint256 sellFee;
        address taxWallet; // deployer's custom tax wallet
    }
    ...
    address public taxWallet; // receives buyFee/sellFee proceeds

    address public taxManager;
    uint256 public buyFeeCeiling;
    uint256 public sellFeeCeiling;
    ...
    event CustomTaxReduced(uint256 newBuyFee, uint256 newSellFee);
    ...
    modifier onlyTaxManager {
        require(msg.sender == taxManager, "Not tax manager");
        _;
    }
    ...
    function initialize(
        ...
        require(config.taxWallet != address(0), "Tax wallet cannot be zero");
        ...
        isFeeExempt[config.taxWallet] = true;
    ...
    function _update(address from, address to, uint256 amount) internal override whenNotPaused {
        ...
        if (canSwap && !inSwap && !isPair[from] && from != owner() && to != owner()) {
            swapBack();
        }
    }
    ...
    function swapBack() private lockTheSwap {
        ...
        if (bnbForPlatform > 0) {
            ...
            if (success) emit PlatformFeeCollected(bnbForPlatform);
        }
        // buyFee and sellFee go to taxWallet (deployer's custom tax wallet)
        if (bnbForBuyFee > 0) {
            (success, ) = payable(taxWallet).call{value: bnbForBuyFee, gas: 50000}("");
        }
        if (bnbForSellFee > 0) {
            (success, ) = payable(taxWallet).call{value: bnbForSellFee, gas: 50000}("");
        }
    ...
    function getBuyFees() public view returns (Fees memory) {
    return buyFees;
}

function getSellFees() public view returns (Fees memory) {
    return sellFees;
}
    ...
    function setTaxWallet(address _taxWallet) external onlyOwner {
        ...
        emit WalletUpdated("tax", _taxWallet);
    }
    ...
    function updateLimits(uint256 _maxTxAmount, uint256 _maxWalletAmount) external onlyOwner {
        require(_maxTxAmount >= (totalSupply() * 10) / 10000, "Max tx too low");
        require(_maxWalletAmount >= (totalSupply() * 10) / 10000, "Max wallet too low");
    ...
    function setSwapTokensAtAmount(uint256 newAmount) external onlyOwner {
        require(newAmount >= (totalSupply() * 1) / 100000, "Amount too low");
        require(newAmount <= (totalSupply() * 50) / 10000, "Amount too high");
    ...
    function setTaxManager(address _taxManager) external onlyOwner {
        require(_taxManager != address(0), "Tax manager cannot be zero");
        taxManager = _taxManager;
        emit TaxManagerUpdated(_taxManager);
    }

    function renounceOwnershipWithTaxControl() external onlyOwner {
        buyFeeCeiling = buyFee;
        sellFeeCeiling = sellFee;
        ownershipRenounced = true;
        _transferOwnership(address(0));
        emit OwnershipRenounced(msg.sender);
    }

    function reduceCustomTax(uint256 _newBuyFee, uint256 _newSellFee) external onlyTaxManager {
        require(ownershipRenounced, "Ownership not yet renounced");
        require(_newBuyFee <= buyFeeCeiling, "Cannot exceed original buy fee");
        require(_newSellFee <= sellFeeCeiling, "Cannot exceed original sell fee");
        require(_newBuyFee <= buyFee, "Can only reduce buy fee");
        require(_newSellFee <= sellFee, "Can only reduce sell fee");
        buyFee = _newBuyFee;
        sellFee = _newSellFee;
        emit CustomTaxReduced(_newBuyFee, _newSellFee);
    }

    function transferTaxManager(address _newTaxManager) external onlyTaxManager {
        require(_newTaxManager != address(0), "New tax manager cannot be zero");
        taxManager = _newTaxManager;
...
// ═══════════════════════════════════════════════════════════════════════════
// FACTORY CONTRACT
// Deployment is free. Unrugpad earns via 0.3% on every buy/sell.
// ═══════════════════════════════════════════════════════════════════════════
...
contract UnrugpadTokenFactory is Ownable {
    ...
    function createToken(
        ...
        require(config.totalSupply <= 1000000000000 * 10**18, "Supply too large");
        ...
        emit TokenCreated(tokenAddress, config.owner, config.name, config.symbol, config.totalSupply);
